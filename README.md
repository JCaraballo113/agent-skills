...
| Skill | Description |
|---|---|
| [agent-rules](./skills/agent-rules/SKILL.md) | Encode working conventions as one rule file per concern in `.claude/rules/` — testing (TDD, happy/negative path, coverage as discovery), migrations, design-system (atomic vocabulary + content extremes), code-review, coding-standard-updates (a gated CODING_STANDARDS.md canon), sub-agents (model tiering), finding-unknowns, agent-summaries (debrief, not changelog) — generalized to the project, any ecosystem. |
...
| [orchestrated-implement](./skills/orchestrated-implement/SKILL.md) | Implements several ready tickets at once: verifies that several ready tickets can run in parallel (blockers, shared outputs, an ownership map of owned vs. shared files), asks which model the forks run on (or lets the orchestrator pick per ticket), forks one implementer per ticket in its own worktree, lands them in a decided order with the spawner reviewing each diff, then puts fresh eyes on the whole landing — `cleanup`, then `/code-review` against the base. Deps in its `skill.deps.json`. |
