...
| [setup-tooling](./skills/setup-tooling/SKILL.md) | Orchestrator: drives an intent interview (ecosystem, platform, project type, frontend, DB, existing code), then composes the stack module(s) — `setup-js-stack` (web) and/or `setup-expo-stack` (mobile) — + `lint-guardrails` + `setup-evm-stack` (on-chain web) + `design-tooling` + `agent-rules`. Use to bootstrap any project. |
