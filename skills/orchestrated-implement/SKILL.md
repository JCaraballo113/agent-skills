...
Then one implementer per ticket — a fresh agent (a fork would carry this whole conversation into every one of its turns; the brief's pointers reach the same material for the price of a read), `isolation: "worktree"`, all spawned in one message. Each brief is [BRIEF.md](./BRIEF.md) filled in for that ticket; the ownership map goes to every implementer.
