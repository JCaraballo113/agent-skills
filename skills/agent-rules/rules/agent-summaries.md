...
- A debrief is what you'd say aloud to a teammate who just walked back to their desk: outcome first in one or two sentences, then detail in the order they'd ask questions — what happened, what changed for them, what needs their decision.
- Register: ASD-STE100 Simplified Technical English — one idea per sentence, short sentences, active voice, one approved meaning per word — with the nouns and verbs taken from the project's ubiquitous language (`CONTEXT.md` where the project keeps one). A project term enters with its meaning in tow the first time.
- Bullets carry the detail, one idea each; paragraphs are reserved for the short outcome up top.
- Name effects, not internals: "maintainers see the label flip within two minutes" lands where "processor #5 converges desired state" doesn't.
...
- The bar before sending: the reader can retell what happened, unaided, after one read. A sentence that needs a second pass gets rewritten, not trimmed.
